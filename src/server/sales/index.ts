import { salesOrderStatuses } from "@/server/shared/constants";
import { formatCategory, toCurrency } from "@/server/shared/format";
import { getSalesOrderItems, getSalesOrders, getSalesProducts } from "./repository";
...
export async function getSalesPageData(period: SalesPeriod = "ALL") {
  ...
  const salesOrders = orders.filter((order) => salesOrderStatuses.includes(order.status) && matchesPeriod(order));
