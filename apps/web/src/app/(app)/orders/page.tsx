...
export default async function OrdersPage({ searchParams }: OrdersPageProps) {
  const { metrics, orders, options } = await getOrdersPageData();
        ...
        <form className="grid gap-3 lg:grid-cols-[minmax(15rem,2fr)_repeat(4,minmax(8rem,0.9fr))_minmax(4.5rem,auto)] lg:items-end">
          ...
          <label className="min-w-0 space-y-1.5">
            ...
            >
              ...
              {(options?.orderStatuses ?? orderStatusOptions).map((status) => (
          ...
          <label className="min-w-0 space-y-1.5">
            ...
            >
              ...
              {(options?.paymentStatuses ?? paymentStatusOptions).map((status) => (
                <option key={status} value={status}>
                  {formatOptionLabel(status)}
                </option>
              ))}
          ...
          <label className="min-w-0 space-y-1.5">
            ...
            >
              ...
              {(options?.fulfillmentTypes ?? fulfillmentOptions).map((type) => (
          ...
          <div className="flex items-center justify-between gap-4 border-b border-[color:var(--line)] px-5 py-4 sm:px-6">
            ...
            <div className="flex items-center gap-3">
              <NewOrderModal
                ...
                orderStatusOptions={options?.orderStatuses ?? orderStatusOptions}
                paymentStatusOptions={options?.paymentStatuses ?? paymentStatusOptions}
                fulfillmentOptions={options?.fulfillmentTypes ?? fulfillmentOptions}
              />
              {activeFilter !== "all" ? (
                <Link
                  href="/orders"
